// --- CONFIGURATION ---
// Change this ID to link this specific course page to a different English profile.
const COURSE_PROFILE_ID = "5-d";
...
function getFlagPath(code) {
  ...
  // Pointing to the global flags folder at the root level of the hub
...
function showResult(qual) {
  ...
  const imgHtml = flag ? `<img src="${flag}" class="flag-result" alt="">` : "";
  ...
  resultArea.innerHTML = `
    <div class="result-box">
        ...
        <div style="margin-top:15px; padding-top:15px; border-top:1px dashed var(--border-color); font-size:0.9rem; color:var(--text-light);">
            <strong>Visa Info:</strong> ${currentCountry.visa_info || "Standard visa rules apply"}
        </div>
    </div>
  `;
}
